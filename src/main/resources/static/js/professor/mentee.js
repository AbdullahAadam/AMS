$(document).ready(function(){
	...
	const $studentCards = $(".student-card");
	const $emptyState = $(".empty-state");
	const $filterWrapper = $(".filter-wrapper");
	if ($studentCards.length === 0) {
	    $emptyState.fadeIn(800);
		$filterWrapper.addClass("hidden-element");
	} else {
		var totalYears = $("#yearFilter").data("total-years"); // Get total years from data attribute
			...
			function filterStudents() {
				   ...
				   if (hasVisibleCards) {
				                  $(".empty-state").hide();
								  $filterWrapper.removeClass("hidden-element");
				              } else {
	...
	$(".view-btn").click(function () {
	        ...
	        window.location.href = `/professor/viewMentee?regNo=${regNo}`;
	    });
